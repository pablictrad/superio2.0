            ...
            <div class="grid grid-cols-1 md:flex ml-4 lg:grid-cols-3 gap-6 mb-6">
                <div>
                    ...
                    <select wire:model.live="idtb_zona" class="w-auto min-w-22.5 border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                        ...
                        @foreach($zonas as $z)
                            @php $z = is_array($z) ? (object)$z : $z; @endphp
                ...
                <div>
                    ...
                    <select wire:model.live="instituto_id" class="w-auto min-w-22.5 border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm {{ empty($institutos) ? 'bg-gray-100' : '' }}">
                        ...
                        @foreach($institutos as $i)
                            @php $i = is_array($i) ? (object)$i : $i; @endphp
                ...
                <div class="mb-4">
                   @if($idtipo_llamado == 2)
                        ...
                        <select wire:model.live="nuevo_rel_carrera_espacio_id"
                            ...
                            @foreach($espacios as $e)
                                @php $e = is_array($e) ? (object)$e : $e; @endphp
                                <option value="{{ $e->id }}">
                                    {{ $e->nombre_espacio }}
                                </option>
                            @endforeach
                    ...
                    <div class="bg-blue-50 p-4 rounded-xl border border-blue-200">
                        ...
                        <div class="bg-white p-3 rounded-lg border border-blue-100 shadow-sm">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                {{-- DATOS DE LA RELACIÓN: solo lectura, se muestran al editar un ítem --}}
                                @if($editando_detalle_index !== null && isset($detalles_agregados[$editando_detalle_index]))
                                @php $det_preview = $detalles_agregados[$editando_detalle_index]; @endphp
                                <div class="md:col-span-2">
                                    <div class="bg-indigo-50 border border-indigo-200 rounded-lg px-3 py-2.5">
                                        <p class="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-2 flex items-center gap-1">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                            Datos de la relación carrera-espacio (solo lectura — editables desde el módulo de relaciones)
                                        </p>
                                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-2">
                                            @if(!empty($det_preview['hora_catedra']))
                                            <div>
                                                <span class="text-[9px] font-black text-indigo-500 uppercase">Hs. Cátedra</span>
                                                <div class="text-[11px] font-bold text-gray-800">{{ $det_preview['hora_catedra'] }}</div>
                                            </div>
                                            @endif
                                            @if(!empty($det_preview['anio']))
                                            <div>
                                                <span class="text-[9px] font-black text-indigo-500 uppercase">Año</span>
                                                <div class="text-[11px] font-bold text-gray-800">{{ $det_preview['anio'] }}°</div>
                                            </div>
                                            @endif
                                            @if(!empty($det_preview['turno']))
                                            <div>
                                                <span class="text-[9px] font-black text-indigo-500 uppercase">Turno</span>
                                                <div class="text-[11px] font-bold text-gray-800">{{ $det_preview['turno'] }}</div>
                                            </div>
                                            @endif
                                            @if(!empty($det_preview['periodo']))
                                            <div>
                                                <span class="text-[9px] font-black text-indigo-500 uppercase">Período</span>
                                                <div class="text-[11px] font-bold text-gray-800">{{ $det_preview['periodo'] }}</div>
                                            </div>
                                            @endif
                                            @if(!empty($det_preview['perfil']))
                                            <div class="col-span-2 sm:col-span-4">
                                                <span class="text-[9px] font-black text-indigo-500 uppercase">Perfil</span>
                                                <div class="text-[11px] text-gray-700 leading-relaxed mt-0.5">{{ $det_preview['perfil'] }}</div>
                                            </div>
                                            @endif
                                        </div>
                                    </div>
