...
new #[Title('Gestión Institucional')] class extends Component
{
    use WithPagination;
    ...
    public function toggleActivoEspacio($id)
    {
        $row = Espacio_curricular::findOrFail($id);
        $row->activo = ! $row->activo;
        $row->save();

        $mensaje = $row->activo ? 'Espacio curricular habilitado.' : 'Espacio curricular deshabilitado.';
        $this->dispatch('toast', variant: 'success', heading: 'Éxito', text: $mensaje);
    }
    ...
    public function toggleActivoCargo($id)
    {
        $row = Cargo::findOrFail($id);
        $row->activo = ! $row->activo;
        $row->save();

        $mensaje = $row->activo ? 'Cargo habilitado.' : 'Cargo deshabilitado.';
        $this->dispatch('toast', variant: 'success', heading: 'Éxito', text: $mensaje);
    }
    ...
    public function toggleActivoCarrera($id)
    {
        $row = Carrera::findOrFail($id);
        $row->activo = ! $row->activo;
        $row->save();

        $mensaje = $row->activo ? 'Carrera habilitada.' : 'Carrera deshabilitada.';
        $this->dispatch('toast', variant: 'success', heading: 'Éxito', text: $mensaje);
    }
    ...
    public function toggleActivoInstituto($id)
    {
        $row = Instituto::findOrFail($id);
        $row->activo = ! $row->activo;
        $row->save();

        $mensaje = $row->activo ? 'Instituto habilitado.' : 'Instituto deshabilitado.';
        $this->dispatch('toast', variant: 'success', heading: 'Éxito', text: $mensaje);
    }
                ...
                <div class="overflow-x-auto rounded-xl border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900 shadow-sm">
                    <flux:table :paginate="$this->espaciosRows">
                        <flux:table.columns>
                            ...
                            <flux:table.column class="!text-zinc-900 font-bold w-28">Estado</flux:table.column>
                            ...
                            @foreach($this->espaciosRows as $row)
                                <flux:table.row :wire:key="'esp-'.$row->getKey()">
                                    ...
                                    <flux:table.cell class="text-sm">
                                        @if($row->activo)
                                            <flux:badge variant="solid" color="lime" size="sm">Habilitado</flux:badge>
                                        @else
                                            <flux:badge variant="solid" color="zinc" size="sm">Deshabilitado</flux:badge>
                                        @endif
                                    </flux:table.cell>
                                    <flux:table.cell class="text-center">
                                        <div class="flex justify-center gap-1">
                                            ...
                                            @if($row->activo)
                                                <flux:button wire:click="toggleActivoEspacio({{ $row->getKey() }})" wire:confirm="¿Deshabilitar este espacio curricular?" variant="ghost" size="xs" icon="x-circle" class="!text-red-500" />
                                            @else
                                                <flux:button wire:click="toggleActivoEspacio({{ $row->getKey() }})" wire:confirm="¿Habilitar este espacio curricular?" variant="ghost" size="xs" icon="check-circle" class="!text-lime-600" />
                                            @endif
                                        </div>
                                    </flux:table.cell>
                                </flux:table.row>
                                ...
                                <flux:table.row :wire:key="'cargo-'.$row->getKey()">
                                    ...
                                    <flux:table.cell class="text-center">
                                        <div class="flex justify-center gap-1">
                                            ...
                                            @if($row->activo)
                                                <flux:button wire:click="toggleActivoCargo({{ $row->getKey() }})" wire:confirm="¿Deshabilitar este cargo?" variant="ghost" size="xs" icon="x-circle" class="!text-red-500" />
                                            @else
                                                <flux:button wire:click="toggleActivoCargo({{ $row->getKey() }})" wire:confirm="¿Habilitar este cargo?" variant="ghost" size="xs" icon="check-circle" class="!text-lime-600" />
                                            @endif
                                        </div>
                                    </flux:table.cell>
                                </flux:table.row>
                                ...
                                <flux:table.row :wire:key="'carrera-'.$row->getKey()">
                                    ...
                                    <flux:table.cell class="text-center">
                                        <div class="flex justify-center gap-1">
                                            ...
                                            @if($row->activo)
                                                <flux:button wire:click="toggleActivoCarrera({{ $row->getKey() }})" wire:confirm="¿Deshabilitar esta carrera?" variant="ghost" size="xs" icon="x-circle" class="!text-red-500" />
                                            @else
                                                <flux:button wire:click="toggleActivoCarrera({{ $row->getKey() }})" wire:confirm="¿Habilitar esta carrera?" variant="ghost" size="xs" icon="check-circle" class="!text-lime-600" />
                                            @endif
                                        </div>
                                    </flux:table.cell>
                                </flux:table.row>
                                ...
                                <flux:table.row :wire:key="'instituto-'.$row->getKey()">
                                    ...
                                    <flux:table.cell class="text-center">
                                        <div class="flex justify-center gap-1">
                                            ...
                                            @if($row->activo)
                                                <flux:button wire:click="toggleActivoInstituto({{ $row->getKey() }})" wire:confirm="¿Deshabilitar este instituto?" variant="ghost" size="xs" icon="x-circle" class="!text-red-500" />
                                            @else
                                                <flux:button wire:click="toggleActivoInstituto({{ $row->getKey() }})" wire:confirm="¿Habilitar este instituto?" variant="ghost" size="xs" icon="check-circle" class="!text-lime-600" />
                                            @endif
                                        </div>
                                    </flux:table.cell>
                                </flux:table.row>
