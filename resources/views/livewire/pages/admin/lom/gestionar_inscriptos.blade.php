...
new class extends Component {
    ...
    private function cargarInscriptos(): void
    {
        $rows = DB::table('inscripciones_llamado')
            ...
            ->when(trim($this->buscar) !== '', function ($q) {
                $busq = trim($this->buscar);
                $q->where(function ($sub) use ($busq) {
                    $sub->where('apellido', 'ilike', "%{$busq}%")
    ...
    /* ── APROBAR / RECHAZAR F2 ────────────────────────────────────── */
    public function aprobarF2(): void
    {
        if (!$this->docInscriptoId) return;

        DB::table('inscripciones_llamado')
            ->where('id', $this->docInscriptoId)
            ->update([
                'f2_estado_id'                => 2, // Aprobado
                'f2_observacion_verificacion' => null,
                // AJUSTAR: confirmar nombre exacto de columna (aparece truncada como "f2_verificado...")
    ...
    public function iniciarRechazoF2(): void
    {
        $this->rechazandoF2 = true;
        $this->obsRechazoF2 = '';
    }

    public function cancelarRechazoF2(): void
    {
        $this->rechazandoF2 = false;
        $this->obsRechazoF2 = '';
    }

    public function confirmarRechazoF2(): void
    {
        if (!$this->docInscriptoId) return;

        $this->validate([
            'obsRechazoF2' => 'required|max:255',
        ], [
            'obsRechazoF2.required' => 'Indicá el motivo del rechazo.',
        ]);

        DB::table('inscripciones_llamado')
            ->where('id', $this->docInscriptoId)
            ->update([
                'f2_estado_id'                => 3, // Rechazado
                'f2_observacion_verificacion' => $this->obsRechazoF2,
                // AJUSTAR: mismo campo que en aprobarF2()
                'f2_verificado_at'            => now(),
                'updated_at'                  => now(),
            ]);
            ...
            <div class="px-6 pt-4 shrink-0">
                <div class="relative">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="buscar"
                        ...
                        class="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-200">
                </div>
            </div>
                ...
                @if(empty($inscriptos))
                    <div class="flex flex-col items-center justify-center py-16 text-center">
                        ...
                        <p class="text-gray-400 font-bold uppercase text-sm tracking-widest">
                            {{ trim($buscar) !== '' ? 'Sin resultados' : 'Sin inscriptos aún' }}
                        </p>
                        <p class="text-gray-300 text-xs mt-1">
                            {{ trim($buscar) !== '' ? 'No hay inscriptos que coincidan con la búsqueda.' : 'Las inscripciones aparecerán aquí cuando los docentes se postulen.' }}
                        </p>
                    </div>
                @else
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="px-3 py-2.5 rounded-lg border border-gray-100 bg-gray-50">
                                ...
                                @if(empty($docInscripto->presento_f2))
                                    <span class="mt-2 inline-block text-[10px] font-black uppercase text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">No presentó</span>
                                @else
                                    @php
                                        $f2Estado = (int) ($docInscripto->f2_estado_id ?? 1);
                                        $f2Clase  = match($f2Estado) { 2 => 'bg-green-100 text-green-700', 3 => 'bg-red-100 text-red-700', default => 'bg-amber-100 text-amber-700' };
                                        $f2Label  = match($f2Estado) { 2 => 'Aprobado', 3 => 'Rechazado', default => 'Pendiente' };
                                    @endphp
                                    <div class="mt-2 flex items-center gap-2 flex-wrap">
                                        <span class="text-[10px] font-black uppercase px-2 py-0.5 rounded-full {{ $f2Clase }}">{{ $f2Label }}</span>

                                        @if($f2Estado === 1)
                                            @if($rechazandoF2)
                                                <div class="w-full flex gap-1 items-start mt-1">
                                                    <input wire:model="obsRechazoF2" type="text" placeholder="Motivo del rechazo..."
                                                        class="flex-1 border border-gray-300 rounded px-2 py-1 text-xs">
                                                    <button wire:click="confirmarRechazoF2"
                                                        class="bg-red-600 hover:bg-red-700 text-white rounded px-2 py-1 text-[9px] font-black uppercase transition">
                                                        Confirmar
                                                    </button>
                                                    <button wire:click="cancelarRechazoF2"
                                                        class="bg-gray-300 hover:bg-gray-400 text-gray-700 rounded px-2 py-1 text-[9px] font-black uppercase transition">
                                                        ✕
                                                    </button>
                                                </div>
                                                @error('obsRechazoF2') <p class="text-red-500 text-[9px] mt-1 w-full">{{ $message }}</p> @enderror
                                            @else
                                                <button wire:click="aprobarF2"
                                                    class="bg-green-600 hover:bg-green-700 text-white rounded px-2 py-1 text-[9px] font-black uppercase transition">
                                                    Aprobar
                                                </button>
                                                <button wire:click="iniciarRechazoF2"
                                    ...
                                    @if(!empty($docInscripto->f2_observacion_verificacion))
                                        <p class="mt-1 text-[10px] text-red-600 italic">Motivo: {{ $docInscripto->f2_observacion_verificacion }}</p>
                                    @endif
                                @endif
                            </div>
                        </div>
