<div>
    <flux:button wire:click="$set('mostrarModalPerfiles', true)">
        Seleccionar
    </flux:button>

    <flux:modal wire:model="mostrarModalPerfiles" class="md:w-7xl">
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                ...
                <flux:button variant="primary" wire:click="nuevoPerfil">+ Agregar Perfil</flux:button>
            </div>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody>
                        @forelse($this->perfilesFiltrados as $per)
                            <tr class="border-t">
                                <td class="px-4 py-3">{{ $per->idtb_perfil }}</td>
                                <td class="px-4 py-3 whitespace-pre-line">{{ $per->nombre_perfil }}</td>
                                <td class="px-4 py-3">
                                    <div class="flex justify-center gap-2">
                                        <flux:button size="sm" variant="primary" wire:click="seleccionarPerfil({{ $per->idtb_perfil }})">Seleccionar</flux:button>
                                        <flux:button size="sm" variant="filled" wire:click="editarPerfil({{ $per->idtb_perfil }})">Editar</flux:button>
                                    </div>
                                </td>
                            </tr>
                        @empty
