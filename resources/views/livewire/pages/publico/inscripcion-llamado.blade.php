 ...
 *  - Presentó F2 (con subida opcional del PDF)
...
new class extends Component {
    ...
    public function buscarDni(): void
    {
        ...
        if ($docente) {
            ...
            $this->localidad  = $docente->localidad ?? '';
    ...
    public function irPaso3(): void
    {
        ...
        $this->validate([
            ...
            'archivoF2' => 'nullable|file|mimes:pdf|max:5120',
        ], [
    ...
    private function resetTodo(): void
    {
        $this->reset([
            ...
            'tieneLegajo','presentoF2','archivoF2',
        ...
        <div class="overflow-y-auto flex-1 px-6 py-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input wire:model="localidad" type="text" placeholder="Ciudad / Localidad"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition">
                </div>
                ...
                <div class="sm:col-span-2">
                    <label class="block text-xs font-black text-gray-600 mb-2 uppercase tracking-wide">
                        ¿Presentó formulario F2 firmado por la escuela?
                    </label>
                    <div class="flex gap-3 mb-3">
                        <label class="flex-1 flex items-center gap-3 p-3 border-2 rounded-xl cursor-pointer transition
                            {{ $presentoF2 ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-gray-300' }}">
                            <input type="radio" wire:model.live="presentoF2" value="1"
                                class="h-4 w-4 text-indigo-600 border-gray-300">
                            <span class="text-sm font-black {{ $presentoF2 ? 'text-indigo-700' : 'text-gray-600' }}">Sí, presenté el F2</span>
                        </label>
                        <label class="flex-1 flex items-center gap-3 p-3 border-2 rounded-xl cursor-pointer transition
                            {{ !$presentoF2 ? 'border-slate-400 bg-slate-50' : 'border-gray-200 hover:border-gray-300' }}">
                            <input type="radio" wire:model.live="presentoF2" value="0"
                                class="h-4 w-4 text-slate-600 border-gray-300">
                            <span class="text-sm font-black {{ !$presentoF2 ? 'text-slate-700' : 'text-gray-600' }}">No presenté el F2</span>
                        </label>
                    </div>
                    ...
                    <div class="bg-indigo-50 border border-indigo-200 rounded-xl p-3">
                        ...
                        @error('archivoF2') <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p> @enderror
                        @if($archivoF2)
        ...
        <div class="flex items-center justify-between px-6 py-4 border-t bg-gray-50 rounded-b-2xl shrink-0">
            ...
            <div class="flex items-center gap-2">
                ...
                <button wire:click="irPaso3"
                    wire:loading.attr="disabled"
                    wire:target="irPaso3,archivoF2"
                    class="bg-indigo-600 hover:bg-indigo-700 text-white font-black px-6 py-2.5 rounded-xl text-sm uppercase shadow-md transition disabled:opacity-50 flex items-center gap-2">
                    <span wire:loading.remove wire:target="irPaso3,archivoF2">
                        Continuar
                        <svg class="w-4 h-4 inline ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"/>
                        </svg>
                    </span>
                    <span wire:loading wire:target="irPaso3,archivoF2" class="flex items-center gap-2">
                        <svg class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                        </svg>
